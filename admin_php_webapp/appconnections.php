...
$response = array();
$ipadd='orderit.000webhostapp.com';
...
if(isset($_POST['userLogin'])){
		...
		if(mysqli_num_rows($query_execute) > 0){
			...
			if($query_status){
				...
				$response['status']=false;
		  		...
		  		$response['error'] = true;
				$response['status']=false;
				$response['message'] = 'Table Already Logged in Someother Device';
		  }
	...
	if(isset($_POST['food_type']) AND isset($_POST['food_category']) ){
		...
		while($row =mysqli_fetch_assoc($query_result)) {
			...
			$temp = array(
				...
				'item_type'=>$ftype,
				'price'=>$row['price'],
				'pic_url'=>'https://orderit.000webhostapp.com/uploads/'.$row['food_image'],
	...
	if(isset($_POST['cust_name']) AND isset($_POST['tab_id'])){
		...
		$query_test = "SELECT isactive FROM tabs WHERE table_id = '$tab_id' AND isdelete = 0";
		...
		if(mysqli_num_rows($query_test_execute) > 0){
			...
			if($trow['isactive']==0){
				...
				if(mysqli_num_rows($query_execute) == 0){
					...
					$query_insert = "INSERT INTO placed_order (slno, customer_name, table_id, order_date, isActive,amount) VALUES (1, '$cust_name', '$tab_id', NOW(), 0,0.0)";
...
else if(isset($_POST['tableStatus'])){
	if(isset($_POST['tab_id']) ){

		$tab_id = $_POST['tab_id'];

$query_find = "SELECT * FROM tabs WHERE (table_id = '$tab_id' AND isdelete=0)";
		...
		if($query_status){
			...
			if($row['isactive'] && $row['islogin']){
				...
				$response['message']="Admin Forced to Logout Please Login Again";
				$response['isActive']=false;
			}
		}
		else{
